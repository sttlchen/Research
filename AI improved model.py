...
# Arrays -> X, targets -> y
# =========================
X_raw = np.stack([np.asarray(a, dtype=np.float32) for a in matched['img_array']])  # (N, 73, 73)
...
# =========================
# Strong linear baseline: PCA -> Ridge with GroupKFold (no leakage)
# =========================
# print("\n=== Linear baseline (PCA -> Ridge, grouped CV) ===")
# gkf = GroupKFold(n_splits=min(N_SPLITS, len(np.unique(groups))))
# # Flatten then scale per-feature (consistent inside folds via Pipeline-like steps below)
# X_flat = X.reshape(len(X), -1)
#
# # We’ll do fold-wise PCA fit inside CV to avoid leakage by manual loop:
# ridge_cv_preds, ridge_cv_true = [], []
# for fold, (tr, va) in enumerate(gkf.split(X_flat, y_raw, groups=groups), 1):
#     Xtr, Xva = X_flat[tr], X_flat[va]
...
#     ridge.fit(Xtr_p, ytr)
#     pred_va = ridge.predict(Xva_p)
#
#     ridge_cv_preds.append(pred_va)
#     ridge_cv_true.append(yva)
#
# y_true_lin = np.concatenate(ridge_cv_true)
# y_pred_lin = np.concatenate(ridge_cv_preds)
# print(f"MAE  : {mean_absolute_error(y_true_lin, y_pred_lin):.3f}")
# print(f"RMSE : {rmse(y_true_lin, y_pred_lin):.3f}")
# print(f"R^2  : {r2_score(y_true_lin, y_pred_lin):.3f}")
